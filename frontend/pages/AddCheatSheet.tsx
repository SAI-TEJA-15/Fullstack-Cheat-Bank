...
import { submitCheatSheet } from '../services/apiService';
import { categories, CheatSheetSection, Command, Category } from '../types';
...
const AddCheatSheet: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await submitCheatSheet({
                title,
                description,
                category,
                authorName,
            ...
            setError(err.message || 'Unable to submit cheat sheet.');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-8 bg-surface p-8 rounded-lg">
                {message && <div className="rounded-md border border-green-500/40 bg-green-500/10 px-4 py-3 text-green-300">{message}</div>}
                ...
                <div className="flex justify-end gap-4">
                    ...
                    <button type="submit" disabled={submitting} className="px-6 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-primary hover:bg-primary-hover disabled:opacity-70">
